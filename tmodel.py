...
# handles translation of words to vectors
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # make matrix of shape seq_len, d_model
        pe = torch.zeros(seq_len, d_model)
        # create vector of shape seq_len, 1
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model)) # uses log instead of exp for stability
        # apply sin to even positions
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)

        pe = pe.unsqueeze(0) # is now (1, seq_len, d_model)
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, self_attentio_block: MultiHeadAttentionBlock, feed_forward_block: FeedForwardBlock, dropout: float) -> None:
        super().__init__()
        self.self_attention_block = self_attentio_block
...
# can use for any transformer applicable purpose, this is set for translation
